...
function addPaginationFooterHandlers() {
    ...
    // sorting
    $('th[data-SortType]').click(function (e) {
        debugger;
        var sortBy = $(this).data('sortingcol');
        var sortOrder = $(this).data('sorttype');

        Pagination.getData(undefined, { 'SortingCol': sortBy, 'SortType': sortOrder });
    });
}
...
function getPaginationFieldParams() {
    ...
    if ($activeSortByEl.length) {
        param['SortingCol'] = $($activeSortByEl).data('SortingCol');

        var sortOrder = $($activeSortByEl).data('SortType');
        param['SortType'] = sortOrder.toUpperCase() === 'ASC' ? 'DESC' : 'ASC';
    }
